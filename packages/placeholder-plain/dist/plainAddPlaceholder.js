 ...
 *
 * @param {InputType & { page: number }} params - Includes all input parameters along with the page number.
 * @returns {Buffer}
 */
const plainAddPlaceholder = ({
  ...
  widgetRect = [0, 0, 0, 0],
  appName = undefined,
  page = 0 // Default is the first page if not specified.
}) => {
  let pdf = (0, _utils.removeTrailingNewLine)(pdfBuffer);
  const info = (0, _readPdf.default)(pdf);

  // Use the 'page' parameter to get the reference of the specified page
  const pageRef = (0, _getPageRef.default)(pdf, info, page); // Assuming pages are 0-indexed internally
  ...
  const pdfKitMock = {
    ref: (input, knownIndex) => {
      ...
      addedReferences.set(index, pdf.length + 1); // +1 for newline
